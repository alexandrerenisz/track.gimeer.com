...
- **`public/tracker.js`**: script vanilla (sem build step) que o site principal
  carrega via `<script src="https://track.gimeer.com/tracker.js" async>`.
...
- **`trck_user_id` NÃO viaja por cookie da nossa API** — como `/api/identify`
  é cross-origin (chamado de `gimeer.com`, servido por
  `track.gimeer.com`), um `Set-Cookie` nosso ficaria no domínio errado
...
- **CORS**: `lib/cors.ts`, lista em `TRACKING_ALLOWED_ORIGINS` (fallback:
  `gimeer.com`/`www.gimeer.com`). `/api/config/public` é liberado geral
...
- Repo privado no GitHub (`gimeer/traqueamento`), a conectar na
  Vercel. Auto-deploy a cada push em `main` (uma vez conectado).
- **Gotcha de plano Hobby + repo privado**: a Vercel bloqueia deploy
  ...
  neste repo está setado pro email do dono do projeto (`eduardo@gimeer.com`)
  especificamente por causa disso — outros repos na máquina não são
  afetados (é `--local`, não `--global`).
...
O site (`gimeer.com`) roda em vários subdomínios (`oferta.gimeer.com`,
`checkout.gimeer.com` etc — páginas geradas por outras ferramentas, fora
deste repo). Isso mudou duas decisões do `tracker.js`/CORS depois do deploy
inicial:

- **`trck_user_id` fica em cookie de domínio raiz** (`domain=.gimeer.com`),
...
- **CORS por wildcard de domínio raiz**, não lista fixa de origins.
  ...
  (default: `*.gimeer.com`) — casa o domínio raiz e qualquer subdomínio,
  sem precisar adicionar cada subdomínio novo manualmente. Testado contra
  tentativa de bypass (`gimeer.com.malicioso.com` corretamente rejeitado).
- **Instalação**: a tag `<script src="https://track.gimeer.com/tracker.js" async>`
...
Este projeto já teve pelo menos três identidades visuais: "Estética High
Level" (`alexandrerenisz.com.br`), depois "Escola de Canais de
Aquisição"/AdvFlow (`escoladecanais.com.br`/`advflowpro.com`), e agora
**Gimeer** (`gimeer.com`) — a migração de escoladecanais/advflowpro pra
gimeer.com foi feita via substituição mecânica de string nos mesmos arquivos
já mapeados na migração anterior (`app/layout.tsx`, `lib/cors.ts`,
`configuracoes/page.tsx`, `test-connection.ts`, `public/tracker.js`,
`.env.example`, `package.json`, e este arquivo). Se aparecer `alexandrerenisz`,
`escoladecanais` ou `advflowpro` de novo em algum lugar, é resíduo que passou
batido, não decisão intencional.

- **Paleta**: gimeer.com não expõe design tokens/CSS nomeado como a LP do
  cliente anterior — só uma cor de marca, extraída direto do `fill` do SVG
  inline da logo no HTML da home (`#bdf688`, um verde-limão claro,
  `hsl(91 86% 75%)`). Vira `--primary` do painel, igual nos dois temas
  (claro e escuro) — como é uma cor clara, `--primary-foreground` é sempre
  escuro (texto branco em cima não teria contraste). `--ring` usa uma
  variante mais escura/saturada do mesmo hue (foco precisa de mais contraste
  que um botão preenchido). Como o site não tem toggle de tema nem um
  "escuro oficial" da marca, o fundo dark do painel é **derivado**: neutro
  bem escuro com leve matiz esverdeado (mesmo hue do primary, baixa
  saturação) — mesma abordagem já usada na paleta anterior (petróleo/
  profundo eram derivados do coral do mesmo jeito). Fundo light é um
  branco levemente esverdeado, coerente com o neutro claro da home do
  cliente. `--amber` continua só como cor utilitária de status (badge
  "partial" em Eventos), sem ligação com marca.
- `public/brand/logo.png` (PNG da logo do cliente anterior) foi **removido**,
  não substituído por outro PNG — `components/layout/logo.tsx` agora renderiza
  o wordmark da gimeer.com como **SVG inline**, com os mesmos `<path>` extraídos
  do SVG da home do cliente (fill único `#bdf688`), em vez de depender de um
  asset raster. Evita precisar de `next/image` + arquivo versionado pra algo
  que já é vetorial na fonte.
- **`app/icon.tsx`/`app/apple-icon.tsx`** (substituem os antigos
  `app/icon.png`/`app/apple-icon.png`): gimeer.com não tem um favicon/selo
  próprio — só o wordmark completo, largo demais pra caber legível num ícone
  quadrado — então o favicon é um monograma "G" gerado em runtime via
  `ImageResponse` (`next/og`), verde da marca sobre fundo escuro sólido
  (mesmo motivo de sempre: favicon transparente some em toolbar clara). A
  troca de PNG estático pra `ImageResponse` (em vez de rasterizar com
  `sharp`, como da vez anterior) foi deliberada: `sharp` nunca foi dependência
  do `package.json`, só usado pontualmente via npx/local naquela ocasião;
  `next/og` já vem embutido no Next.js, então gerar o ícone assim não exige
  instalar nada.
- **`components/layout/logo.tsx`**: o wordmark é só o verde da marca sobre
  fundo transparente (sem texto em cor separada, diferente da logo anterior)
  — mesmo assim, verde-limão claro sobre o `--background` claro do painel
  (também esverdeado e claro) teria contraste baixo. Por isso o componente
  continua envolvendo o SVG num chip com fundo escuro fixo (hardcoded, não
  a variável de tema — agora `#0F1712`, análogo ao antigo `#0A2B38`),
  garantindo legibilidade nos dois temas sem precisar de uma segunda variante
  de asset. Usado em `Sidebar`, `MobileNav` e `LoginPage`.
...
- **`window.trckCheckoutUrl(url)`** (nova função no `tracker.js`): LPs como
  `operacao.gimeer.com` usam um popup de lead (nome/whatsapp/email)
  ...
  checkout (`checkout.gimeer.com`, domínio raiz **diferente** de
  `gimeer.com` — o cookie de domínio raiz não ajuda aqui). `trckCheckoutUrl`
...
- **Pendente do lado da LP**: a recipe original só mandava
  ...
  (nomes de variável conforme o bundle real de `operacao.gimeer.com`)
...
Identidade visual alinhada com gimeer.com (paleta + logo real + favicon).
